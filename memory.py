...
class MemoryManager:
    ...
    def initialize_db(self):
        ...
        if not os.path.exists(self.schema_path):
            log_event("CRITICAL", f"Şema dosyası bulunamadı: {self.schema_path}", __name__)
            return
    ...
    # --- DÜZELTME: SQL SORGUSU ŞEMA İLE EŞLEŞTİRİLDİ ---
    def promote_to_memory(self, user_query, bot_response):
        ...
        try:
            # Şemada 'timestamp' değil 'created_at' var. Düzeltildi.
            self.cursor.execute("""
    ...
    def read_from_memory(self, query: str) -> str:
        ...
        try:
            # Şemada 'created_at' kullanılıyor. Düzeltildi.
            self.cursor.execute('''
